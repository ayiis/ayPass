...
class Record(object):
    ...
    def create(self, title, content, content_type, create_datetime):
        ...
        content_info = yield mongodb.DBS["aypass"]["contents"].insert_one({
    ...
    @tornado.gen.coroutine
    def remove(self, _id):
        content_info = yield mongodb.DBS["aypass"]["contents"].delete_one({
            "_id": ObjectId(_id),
        })
    ...
    @tornado.gen.coroutine
    def query(self, _id):
        content_info = yield mongodb.DBS["aypass"]["contents"].find_one({
            "_id": ObjectId(_id),
        })
    ...
    def query_list(self, query, content_type="text"):
        ...
        content_info = yield mongodb.DBS["aypass"]["contents"].find(
            query_data,
            {
                "_id": 1,
                "title": 1,
    ...
    def update(self, _id, title, content, update_datetime):
        ...
        content_info = yield mongodb.DBS["aypass"]["contents"].update_one({
            "_id": ObjectId(_id),
        }, {
            "$set": {
                "title": title,
    ...
    def transfer_to_new_user(self, new_user):
        user_contents = yield mongodb.DBS["aypass"]["contents"].find({
            "username": self.user.username,
        }).to_list(length=None)
        if user_contents:
            update_list = []
            for item in user_contents:
                ...
                update_list.append(mongodb.DBS["aypass"]["contents"].update_one({"_id": item["_id"]}, {"$set": item}))
