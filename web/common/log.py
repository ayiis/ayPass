...
class Log(object):
    ...
    def create(self, log_type, target, create_datetime):
        ...
        content_info = yield my_mongodb.MONGODBS["aypass"]["logs"].insert_one({
    ...
    @tornado.gen.coroutine
    def query_list(self):
        content_info = yield my_mongodb.MONGODBS["aypass"]["logs"].find({
            "username": self.user.username,
        }, {
            "_id": 0,
            "username": 0,
    ...
    def transfer_to_new_user(self, new_user):
        yield my_mongodb.MONGODBS["aypass"]["logs"].update_many({ "username": self.user.username }, {"$set": {"username": new_user.username }})
